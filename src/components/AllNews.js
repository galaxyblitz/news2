...
import {Box, Center, Flex, SimpleGrid, Spinner, Container} from "@chakra-ui/react";
import {useStore} from "../contexts/AuthContext";
import {handlerFetchNews} from "../utils/handlers";
import {Contract, ethers} from "ethers";
import {ABI, CHAIN_RPC, CONTRACT, IPFS_GET_LINK} from "../utils/Constants";
...
function AllNewsPage() {
    ...
    const handlerFetchNews = async () => {
        setLoading(true)
        const array = [];
        const ethersProvider = new ethers.providers.JsonRpcProvider(CHAIN_RPC);
                ...
                : <>
                    {(content.length > 0)
                        ? <>
                            ...
                            <SimpleGrid mr={'40px'} rowGap={'0px'} columnGap={'60px'} columns={[1, 1, 2, 3, 3]}>
                                {content && content.map((item, index) => <Box w='110%'
                                                                              key={item.data.title + '_' + index}>
